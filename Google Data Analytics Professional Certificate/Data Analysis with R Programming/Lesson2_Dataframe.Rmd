...
```{r}
names <- c("Darwin", "Einstein", "Curie", "Turing")
```
...
```{r}
age <- c(150, 100, 110, 80)
```

With these two vectors, you can create a new data frame called `great_minds`:

```{r}
great_minds <- data.frame(names, age)
```
...
```{r}
mutate(great_minds, age_in_20 = age + 20)
```
